...
#Total sales of a product refers to the total revenue generated from
# selling that specific product over a given period
...
group by category
"""
# make the payment_value col. aggregated first so as to apply round function to it
...
plt.bar(df["state"], df["customer_count"]) # x-axis, y-axis
...
ax.bar_label(ax.containers[0]) # labelling each bar with its count
